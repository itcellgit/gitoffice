            ...
            <div class=" block ltr:ml-auto rtl:mr-auto my-auto">
                    ...
                    <div id="hs-medium-modal" class="hs-overlay hidden ti-modal">
                        <div class="hs-overlay-open:mt-7 ti-modal-box mt-0 ease-out md:!max-w-2xl md:w-full m-3 md:mx-auto">
                            ...
                            <form action="{{route('ESTB.staff.staffshares.store',$staff->id)}}" method="post">
                                @csrf
                                <div class="ti-modal-body">
                                    {{-- <div class="max-w-sm space-y-3">
                                        <label for="with-corner-hint" class="ti-form-label font-bold">Staff Name:<span class="text-red-500">*</span></label>
                                    ...
                                    <div class="max-w-sm space-y-3">
                                        <label for="with-corner-hint" class="ti-form-label">Share Id:<span class="text-red-500">*</span> </label>
                                        <input type="text" name="shares_id" class="ti-form-input" required placeholder="Share Id">
                                    </div>
                                    <div class="max-w-sm space-y-3">
                                        <label for="with-corner-hint" class="ti-form-label">Amount:<span class="text-red-500">*</span> </label>
                                        <input type="text" name="amount" class="ti-form-input" required placeholder="Enter Share Amount">
                                    </div>
                                    <div class="max-w-sm space-y-3">
        ...
        <div class="table-bordered rounded-sm ti-striped-table ti-custom-table-head overflow-auto">
           <table id="table" class="ti-custom-table ti-custom-table-head whitespace-nowrap">
            <thead class="bg-gray-50 dark:bg-black/20">
                <tr class="">
                    ...
                    <th scope="col" class="dark:text-white/80">Share Id</th>
         ...
         <tbody class="">
            ...
            @php
                $i = 1;
            @endphp
            @if($staff->staffshares!=null)
            @forelse($staff->staffshares as $share)
            <tr class="">
                <td>{{ $i++ }}</td>
                {{-- <td>
                    @foreach($staff as $s)
                ...
                <td><span>{{$share->member_id}}</span></td>
                <td><span>{{$share->shares_id}}</span></td>
                <td><span>{{$share->amount}}</span></td>
                <td><span>{{$share->start_date}}</span></td>
                <td><span>{{$share->end_date}}</span></td>
                <td><span>{{$share->status}}</span></td>
                <td class="font-medium space-x-2 rtl:space-x-reverse">
                {{-- <div class="hs-tooltip ti-main-tooltip">
                     <a href="{{route('ESTB.stafflic_transactions.show',[$staff->id,$share->id])}}"
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16"><path d="M16.7574 2.99666L14.7574 4.99666H5V18.9967H19V9.2393L21 7.2393V19.9967C21 20.5489 20.5523 20.9967 20 20.9967H4C3.44772 20.9967 3 20.5489 3 19.9967V3.99666C3 3.44438 3.44772 2.99666 4 2.99666H16.7574ZM20.4853 2.09717L21.8995 3.51138L12.7071 12.7038L11.2954 12.7062L11.2929 11.2896L20.4853 2.09717Z"></path></svg>
                        ...
                        <div id="qual_edit_modal{{$i}}" class="hs-overlay hidden ti-modal">
                            <div class="hs-overlay-open:mt-7 ti-modal-box mt-0 ease-out md:!max-w-2xl md:w-full m-3 md:mx-auto">
                                ...
                                <form  action="{{route('ESTB.staff.staffshares.update',[$staff->id,$share->id])}}" method="post">
                                    @csrf
                                    @method('patch')
                                    <div class="ti-modal-body">
                                        ...
                                        {{-- <div class="max-w-sm space-y-3 pb-6">
                                         ...
                                         <div class="max-w-sm space-y-3 pb-6">
                                            ...
                                            <input type="text" name="member_id" class="ti-form-input" required  value="{{$share->member_id}}">    
                                        </div>
                                        <div class="max-w-sm space-y-3 pb-6">
                                            <label for="with-corner-hint" class="ti-form-label font-bold">Share Id:</label>
                                            <input type="text" name="shares_id" class="ti-form-input" required  value="{{$share->shares_id}}">    
                                        </div>
                                        <div class="max-w-sm space-y-3 pb-6">
                                            <label for="with-corner-hint" class="ti-form-label font-bold">Amount:</label>
                                            <input type="text" name="amount" class="ti-form-input" required value="{{$share->amount}}">    
                                        </div>
                                        <div class="max-w-sm space-y-3 pb-6">
                                            <label for="with-corner-hint" class="ti-form-label font-bold">Start Date:</label>
                                            <input type="date" name="start_date" class="ti-form-input" required  value="{{$share->start_date}}">    
                                        </div>
                                        <div class="max-w-sm space-y-3 pb-6">
                                            <label for="with-corner-hint" class="ti-form-label font-bold">End Date:</label>
                                            <input type="date" name="end_date" class="ti-form-input"  value="{{$share->end_date}}">    
                                        </div>
                                        {{-- <div class="max-w-sm space-y-3 pb-6">
                                            <label for="with-corner-hint" class="ti-form-label font-bold">Status: </label>
                                            <select name="edittype" class="form-control edittype">
                ...
                <div class="hs-tooltip ti-main-tooltip">
                <form action="{{ route('ESTB.staff.staffshares.destroy',[$staff->id,$share->id]) }}" method="post">
