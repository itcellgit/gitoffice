                    ...
                    <div class="box-body">
                        <div class="flex">
                            ...
                            <div class="flex-1">
                                ...
                                <div class="flex justify-between items-center">
                                    <h5 class="mb-0 text-2xl font-semibold text-gray-800 dark:text-white">{{
                                        $counts['total_regular_issues'] }}</h5>
                                    <span class="text-success badge bg-success/20 rounded-sm p-1"><i
                                            class="ti ti-trending-up leading-none"></i> +0.36%</span>
                                </div>
                                <span class=" text-gray-500 dark:text-white/70 text-xs"></span>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="box-body">
                        <div class="flex">
                            ...
                            <div class="flex-1">
                                ...
                                <div class="flex justify-between items-center">
                                    <h5 class="mb-0 text-2xl font-semibold text-gray-800 dark:text-white">{{
                                        $counts['resolved_unusual_issues'] }}</h5>
                    ...
                    <div class="box-body">
                        <div class="flex">
                            ...
                            <div class="flex-1">
                                ...
                                <div class="flex justify-between items-center">
                                    <h5 class="mb-0 text-2xl font-semibold text-gray-800 dark:text-white">{{
                                        $counts['resolved_regular_issues'] }}</h5>
                                    <span class="text-success badge bg-success/20 rounded-sm p-1"><i
                                ...
                                <div class="flex">
                                    <h5 class="box-title my-auto">
                                        <svg xmlns="http://www.w3.org/2000/svg"
                                            class="icon icon-tabler icon-tabler-building-church" width="32" height="32"
                                    ...
                                    <div class="avatar-container flex py-4">
                                        <div class="avatar-wrapper flex items-center">
                                            <div
                                                class="avatar rounded-sm p-1 bg-green-500 border-gray-900 border-2 w-6 h-6">
                                            </div>
                                        ...
                                        <div class="avatar-wrapper flex items-center mx-2">
                                            <div
                                                class="avatar rounded-sm p-1 bg-red-300 border-gray-900 border-2 w-6 h-6">
                                            </div>
                            ...
                            <div class="box-body">
                                <form action="{{ route('Staff.Non-Teaching.view') }}" method="get">
                                    ...
                                    <button id="exportToExcel"
                                        class="bg-green-500 text-white px-4 py-2 rounded-md focus:outline-none hover:bg-green-600">Export
                                        to Excel</button>
                                </div> -->
                                        <table id="grievance_table"
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    <td
                                                        class="font-medium space-x-2 rtl:space-x-reverse {{ (count($issue->issue_timeline) > 0 && $issue->issue_timeline->last()->status == 'resolved') ? 'bg-green-500' : ((count($issue->issue_timeline) > 0) ? '' : 'bg-red-300') }} ">
                                                        {{ $i++ }}</td>
                                                    <td
                                                        class="font-medium space-x-2 rtl:space-x-reverse {{ (count($issue->issue_timeline) > 0 && $issue->issue_timeline->last()->status == 'resolved') ? 'bg-green-500' : ((count($issue->issue_timeline) > 0) ? '' : 'bg-red-300') }}">
                                                        {{ $issue->usn }}</td>
                                                    <td
                                                        ...
                                                        {{ $issue->exam_section_issue->issues ?? '' }} - {{
                                                        $issue->exam_section_issue->remarks ?? '' }}
                                                        @else
                                                    ...
                                                    <td
                                                        class="font-medium space-x-2 rtl:space-x-reverse {{ (count($issue->issue_timeline) > 0 && $issue->issue_timeline->last()->status == 'resolved') ? 'bg-green-500' : ((count($issue->issue_timeline) > 0) ? '' : 'bg-red-300') }}">
                                                        @if ($issue->exam_section_issue &&
                                                        $issue->exam_section_issue->staff)
                                                    ...
                                                    <td
                                                        ...
                                                        <div class="hs-tooltip ti-main-tooltip">
                                                            <a href="{{ route('Staff.Non-Teaching.issue_timeline.show',[$staff->id,$issue->id]) }}"
...
<script>
    $(document).ready(function() {
        $('#exportToExcel').click(function() {
            ...
            var ws = XLSX.utils.table_to_sheet(document.getElementById('grievance_table'));

            // Append the sheet to the workbook
            XLSX.utils.book_append_sheet(wb, ws, "Grievances");

            // Export the workbook to Excel
            XLSX.writeFile(wb, 'Student_Issues.xlsx');
        });
    });
</script>
@endsection -->
...
<script>
    $(document).ready(function () {
        ...
        $('#exportToExcel').on('click', function () {
            ...
            saveAs(blob, 'ntstaff_student_issues.xlsx');
        });
    });
</script>
