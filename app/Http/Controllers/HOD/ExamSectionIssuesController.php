...
class ExamSectionIssuesController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        return view('HOD.examsectionissues', compact('examsectionissues','staff'));
    }
    ...
    public function store(StoreExamSectionIssuesRequest $request)
    {
        ...
        if($categoryinsertedId > 0){
            $status = 1;
            return redirect('/HOD/examsectionissues')->with('status', $status);
        }else{
            $status = 0;
            return redirect('/HOD/examsectionissues')->with('status', $status);
        }
    ...
    {
        return view('HOD.issue_timeline',compact('examSectionIssues'));
    }
    ...
    public function destroy(exam_section_issue $examSectionIssues)
    {
        ...
        return redirect('/HOD/examsectionissues');
    }
}
