...
class LeaveStaffApplicationsController extends Controller
{
    ...
    public function index()
    {
       ...
       //dd($staff_leave_entitlements);
        ...
        //fetch the staff members of the department teaching staff - tea{{  }}}}}}}}g data and
        ...
        $dept_staff=department::with(['staff'=>function($q) use ( $staff_id_from_user,$staff_emp_type){
            ...
            ->distinct('department_staff.department_id')
       ...
       //dd($staff);
        ...
        else
        {
            ...
            ($staff->latestassociation()->first()->asso_name=='Confirmed'||
            $staff->latestassociation()->first()->asso_name=='Promotional Probationary'))
            {
                $leaves=DB::table('leaves')->join('leave_rules','leave_rules.leave_id','=','leaves.id')
                                           ->whereNull('leave_rules.max_time_allowed')
                                           ->where('vacation_type','vacational')
    ...
    public function store(Storeleave_staff_applicationsRequest $request)
    {
       ...
       // dd($request);
        ...
        $check_additional_non_vacational=DB::table('designation_staff')->whereIn('designation_id',function($q){
            $q->select('id')->from('designations')->where('isadditional',1)->where('isvacational','Non-Vacational');
        })
        ...
        {
            //dd();
            $hod=user::select('id as user_id')
                        ->where('Role','Principal')->first();
        }
        else
        {
       ...
       // dd($result);
        ...
        if($result === "success")
        {
            ...
            // Recommender is HoD of the staff whoi is applying the leave
            // if(){
            //     $leave_application->approver = $principal->id; // Approver is principal if it crosses 5 days.
            // }else{
            //      // approver is Dean , if its below 5 days.
            // }
    ...
    //All functions for Non-Teaching Leave Application in Non-Teaching login
    ...
    //method to validate the leaves as per the leave rules and leave combination allowed in Non-Teaching
    public function nt_leave_validateleave(request $request, staff $staff)
    {
     ...
     //custom functions in Non-Teaching Login
     public function nt_leave_hollidayrh_events(){
    ...
    //Function for my leave events in Non-Teaching
    public function nt_leave_myleaveevents(){
    ...
    //for checking whether an eventt is there on the clicked date on the calender in Non-Teaching.
    public function nt_leave_checkhasleaveEvent(Request $request){
        $date = $request->input('date');
    ...
    //function for check any departmentpersonleave in Non-Teaching
    public function nt_leave_checkanydeptpersononleave(Request $request){
        $date = $request->input('date');
        //dd($date);
        $user = Auth::user();
        $staff = staff::where('user_id','=',$user->id)->first();
    ...
    public function nt_leave_checkhasRH (Request $request){
        $date = $request->input('date');
    ...
    //for fetching the specific holiday and rh for the clicked date (Ajax call) in Non-Teaching.
    public function nt_leave_fetchholidayrhevents(Request $request){
    ...
    //for fetching the leave events to display on click of the event in Non-Teaching.
    public function nt_leave_fetchmyleaveevents(Request $request){
        $date = $request->input('date');
        //dd($date);
        $user = Auth::user();
        $staff = staff::where('user_id','=',$user->id)->first();
     ...
     //for cancellation of the leave in Non-Teaching.
    function nt_leave_cancel_myleave(Request $request){
    ...
    //for editing the applied leave in Non-Teaching .
    public function nt_leave_edit_myleave(Request $request){
        $application_id = $request->input('application_id');
