...
class Pl_ExamSectionIssuesController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        return view('PRINCIPAL.examsectionissues', compact('examsectionissues','staff'));
    }
    ...
    public function store(StoreExamSectionIssuesRequest $request)
    {
        ...
        if($categoryinsertedId > 0){
            $status = 1;
            return redirect('/PRINCIPAL/examsectionissues')->with('status', $status);
        }else{
            $status = 0;
            return redirect('/PRINCIPAL/examsectionissues')->with('status', $status);
        }
    ...
    {
        return view('PRINCIPAL.issue_timeline',compact('examSectionIssues'));
    }
    ...
    public function destroy(exam_section_issue $examSectionIssues)
    {
        ...
        return redirect('/PRINCIPAL/examsectionissues');
    }
}
